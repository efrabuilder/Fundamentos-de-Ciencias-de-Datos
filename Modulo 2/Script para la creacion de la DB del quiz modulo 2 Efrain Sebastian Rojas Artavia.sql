...
nota decimal(4,2) not null, -- Decimal is used when you need to define variables that are decimal, in this case the accepted range is before the comma four digites and after the comma just two. I'd decided to use decimal because, float usually represent the decimal with a lot of digites and the variable "nota" needs an exact value, so decimal is used when you have to define range and the min and max that is possible insert. 
