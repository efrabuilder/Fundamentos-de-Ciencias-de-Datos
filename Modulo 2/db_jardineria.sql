DROP DATABASE IF EXISTS jardineria; -- comando para borrar la db si existiera, command to delete the db if it's already existed in memory.
GO

CREATE DATABASE db_jardineria; -- command to create db.
go

USE db_jardineria; --define the location of tables and data.
go

CREATE TABLE oficina (  -- command to create table.
...
select codigo_oficina, ciudad, telefono --command to find a specific data from a table, where means a filter.
from oficina
where pais = 'EEUU'
go
